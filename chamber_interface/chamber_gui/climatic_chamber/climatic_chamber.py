...
class ClimaticChamber:
    ...
    def is_connected(self):
        ...
        """
        try:
            res = requests.get(f"http://{self.ip}:{self.port}{self._curr_temp_uri}", timeout=3)
    ...
    def get_current_temp(self, timeout=None):
        ...
        res = requests.get(f"http://{self.ip}:{self.port}{self._curr_temp_uri}", timeout=timeout)

        return float(res.text)


    def get_target_temp(self, timeout=None):
        """Get chamber inside current temperature
        ...
        res = requests.get(f"http://{self.ip}:{self.port}{self._target_temp_uri}", timeout=timeout)

        return float(res.text)


    def set_target_temp(self, temp, timeout=None):

        if timeout is None:
            timeout = 2

        res = requests.post(f"http://{self.ip}:{self.port}{self._target_temp_uri}", str(temp), timeout=timeout)
